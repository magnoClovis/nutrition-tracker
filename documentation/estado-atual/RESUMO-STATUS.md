...
- **[C14-C-PROFILE-GATE] Em correção (12/09/2026) — Chat: Trofia-Principal.** Durante a validação real do Pages após o PR #189, uma conta antiga e completa recebeu indevidamente o modal de criação “Completar perfil nutricional”; F5 restaurou o estado e os três fluxos de IA funcionaram. A causa difere da C14-A: `initializeAppCheck()` era confundido com token pronto, um dummy token não vazio podia ser aceito e uma leitura `getDoc()` bem-sucedida a partir de cache antigo era tratada como ausência real. O hotfix exige token App Check válido, decide o perfil somente por leitura confirmada no servidor, apresenta erro recuperável em indisponibilidade e restringe o modal ao evento explícito de criação de conta. Fase 4 e enforcement permanecem abertos até validação do PR e do Pages.
...
- **[Câmera embutida — C4a] Em andamento (12/09/2026) — Chat: Trofia-UIUX.** O draft PR #192 adiciona permissão explícita, timeouts, retry e limpeza tardia do Camera Preview; interrupção por background e Voltar; bloqueio de orientação e estabilidade de scroll, sem zoom/flash/troca de câmera. No Galaxy físico, permissão concedida/negada, preview confinado, rotação solicitada, Home, Voltar, três ciclos de cancelamento e captura Base64 foram exercitados; todos liberaram o cliente nativo. O APK paralelo `.c4proof` foi removido e nenhum APK/AAB foi publicado. Gate local final: 105/105 focados, 1.337/1.337 unitários e cutover 60/60, todos sem skip; falta o CI autenticado e a aprovação/merge antes de declarar a subfatia concluída.
...
- **Achado da fase 4:** a validação manual do Pages confirmou os três fluxos de IA após F5, mas revelou uma corrida de bootstrap capaz de mostrar o modal exclusivo de criação a uma conta antiga. O hotfix `C14-C-PROFILE-GATE` está em andamento e bloqueia AAB/enforcement até ser validado.
