...
- **O que foi feito:** os horários dos artefatos comprovaram que o gate local colidiu com o CI `35281945540` na mesma conta. O draft #231 agora contém lock local, recusa diante de CI já ativo, workflow manual de lease no mesmo grupo remoto, testes e guia operacional. O primeiro CI do novo HEAD revelou uma fragilidade independente no teste visual de `SearchableChoiceField`; o roteiro agora fecha e confirma o editor de fixture antes de navegar. O recorte passou 3/3, a suíte integral passou 1.410 unitários, 103 legado + 8 skips estruturais, 111 Vite e 60 cutover, e o CI autenticado final `35360391365` passou integralmente. A F1 está pronta para revisão/merge; a aquisição automática permanece corretamente reservada à F2.
