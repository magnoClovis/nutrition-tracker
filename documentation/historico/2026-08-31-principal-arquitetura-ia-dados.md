...
- **Código:** `C08-E`.
- **Status:** em andamento; correção técnica mesclada, validação física pós-hotfix pendente.
...
- **Tempo decorrido:** 11 h 19 min 54 s, do primeiro commit `194d474` (25/09/2026 22:00:20 UTC) ao merge `6293899` (26/09/2026 09:20:14 UTC).
- **Minutos de CI:** 128 min 25 s nas três tentativas concluídas (leves: 2 min 47 s; pesados: 125 min 38 s), medidos do início à atualização terminal dos runs. A primeira tentativa somou 31 min 27 s (leves 53 s; pesado 30 min 34 s) e falhou; a segunda somou 32 min 40 s (leves 1 min; pesado 31 min 40 s) e passou integralmente; a terceira, documental, somou 64 min 18 s (leves 54 s; pesado 63 min 24 s) e também passou.
...
- **O que foi feito:** em 25/09/2026, a fatia foi iniciada a partir da `origin/main` `a398315` em worktree isolado. A leitura do código confirmou quatro pontos que o trabalho precisava fechar: o Durable Object recebia e persistia o UID Firebase bruto; os limites individuais atuais eram barreiras antiabuso por minuto e não representavam tiers comerciais; a chamada ao Gemini não tinha deadline nem leitura limitada da resposta; e a observabilidade do Worker estava desativada. A implementação transforma o UID por HMAC-SHA-256 antes do Durable Object, apaga uma única vez as janelas antigas com UID bruto na migração v3 sem zerar o contador global diário, modela tiers arbitrários por claim verificada/configuração de servidor com enforcement comercial em `observe`, limita Gemini a 40 s/128 KB/1.200 tokens e aceita apenas métricas sanitizadas. A documentação oficial atual da Cloudflare confirmou que Workers Logs retém somente 3 dias no plano Free ou 7 dias no Paid, enquanto Analytics Engine retém três meses; para cumprir o teto aprovado sem fingir uma configuração inexistente, logs operacionais usam a janela nativa curta e o Durable Object guarda somente agregados diários sem identificador, removidos por alarme em no máximo 30 dias. Passaram o preflight, 1.454/1.454 unitários, 44/44 testes Node do Worker, 9/9 testes runtime do Durable Object, dry-run do Wrangler, 52/52 casos locais do smoke legado e 52/52 do Vite com 63 skips autenticados/visuais esperados por matriz sem credenciais locais, além do cutover 60/60. O primeiro CI do PR #257 revelou dois gates antigos, não uma regressão do Worker novo: o verificador de produção ainda esperava `observe` apesar do enforcement C14-C5 já ativo, e o smoke de despensa chamava o Worker sem App Check. Ambos foram tornados fail-closed para o estado real: PRs agora provam `enforce`, e o smoke troca o segredo debug registrado por token curto e envia `X-Firebase-AppCheck`, sem imprimir segredo/token; 11/11 testes focados cobrem os contratos. O segundo e o terceiro CI autenticado remoto passaram integralmente após a correção. O PR #257 foi mesclado em `6293899` em 26/09. Na conclusão operacional, o perfil Wrangler correto foi ativado apenas nesta worktree e um dry-run confirmou `APP_CHECK_MODE=enforce` e `AI_TIER_MODE=observe`. O código da main `312403e` e um segredo HMAC aleatório de 48 bytes foram preparados em versões sem tráfego, sem expor o valor ou usar `secret put` com publicação imediata. A versão `179df5a8-af1b-4d0a-a76a-6bedd32ed6f1` substituiu a versão anterior `cf6f8d82-566c-483f-9fb0-2a587dda0dab` em 100% às 10:27:06 UTC. O segredo foi confirmado apenas pelo nome. A verificação sem App Check recebeu o `401` contratual; o smoke de despensa autenticado em produção terminou verde na tentativa 2 do run `36220582228`. Em smoke independente, uma conta descartável nova obteve token App Check, recebeu HTTP 200 e contrato válido em texto e imagem, e foi excluída com HTTP 200 no `finally`. Um evento controlado sem credenciais mostrou log customizado contendo exclusivamente as sete dimensões permitidas, com persistência correspondente via `recordMetric` no Durable Object; o streaming foi encerrado, sem inspecionar usuários reais. A amostra do tail inclui metadados efêmeros de transporte do próprio serviço Cloudflare, mas o log customizado e a linha agregada não contêm UID, token, prompt, imagem nem resposta. Rollback da versão antiga não reverteria dados do Durable Object e voltaria a armazenar UID bruto, portanto não foi executado sem regressão. A falha visual inicial da câmera não se repetiu no CI e continua atribuída à triagem UI/UX, fora desta fatia.
- **Alinhamento:** 100%. Implementação, controles de segurança, publicação e gates reais correspondem ao plano aprovado. A correção dos gates legados de CI apenas os alinhou ao enforcement C14-C5 já vigente; impacto positivo, sem alterar a política do Worker. O limite comercial permanece deliberadamente em `observe`.
- **PRs/commits relacionados:** PR #257; commits `194d474`, `e75d60d` e `bce367f`; merge `6293899`; base `a398315`. Na primeira tentativa, os gates de produção `36194586861`/`36194586893` revelaram configuração pré-enforcement residual; o CI pesado `36194586909` aprovou 118 testes Playwright e falhou em um teste visual mobile de CAM-RED-6 (`closeHeight` 47,58 px contra mínimo de 48 px). Em 26/09/2026, o reteste local completo `npm test` terminou com código zero, incluindo cutover 60/60. A segunda tentativa remota passou integralmente: preflight `36220582214`, pantry `36220582228`, App Check `36220582185` e CI autenticado `36220582177`; o CI contou 1.465 unitários, 44 testes Node do Worker, 74 de Functions, smoke legado 111 aprovados + 8 skips estruturais esperados e Vite 119 aprovados. A terceira tentativa no commit documental `bce367f` também passou: preflight `36222492836`, pantry `36222492863`, App Check `36222492845` e CI autenticado `36222492819`. O PR documental #261 (merge `312403e`) registrou as métricas pós-merge do #257: Tempo decorrido 48 min 22 s; Minutos de CI 25 s (leve: 25 s; pesado: 0 min, não aplicável a PR somente documental), copiados à descrição do próprio #261. Após o deploy, a tentativa 2 do pantry `36220582228` passou contra produção. A falha visual anterior não se repetiu, sem correção de câmera nesta fatia, e sua causa segue para triagem da UI/UX. — **Chat:** Trofia-Principal.
...
- **Minutos de CI:** 38 min 35 s no PR #265 até este SHA (32 s leves + 38 min 3 s pesados); a C14-F2 permanece em andamento.
- **Propósito:** reduzir privilégios e dependências somente depois de conhecer o estado administrativo real.
- **O que se planeja fazer:** reconfirmar o inventário administrativo de IAM, contas de serviço, invocadores Cloud Run, Cloud Tasks, Scheduler e Artifact Registry, além dos nomes/tipos de segredos do Worker e das dependências por lockfile. Classificar riscos e propor ordem segura de privilégio mínimo e atualizações, sem criar contas, alterar permissões, atualizar pacotes ou publicar serviços antes da avaliação do inventário.
- **Recursos/arquivos principais envolvidos:** Google Cloud Resource Manager IAM, Cloud Run IAM, Cloud Tasks, Cloud Scheduler, Artifact Registry, Firebase Functions, Cloudflare Wrangler, `functions/src/index.js`, manifests/lockfiles da raiz/Worker/Functions, `npm audit` e `documentation/estado-atual/C14_F2_PRE_INVENTARIO_IAM_DEPENDENCIAS.md`.
- **O que foi feito:** a F2 começou após o fechamento operacional e documental da F1 (PRs #257/#261/#262), reaproveitando o inventário preparatório do PR #258 sem editar qualquer recurso de produção. `firebase functions:list --json` reconfirmou as três Functions Gen2 `ACTIVE`, Node.js 22, na conta padrão de Compute Engine. A política IAM do projeto ainda concede `roles/editor` a essa identidade, risco principal porque o callable, o processador e o reconciliador executam tarefas administrativas diferentes. As políticas Cloud Run confirmaram `allUsers` no callable, nenhuma binding própria no processador de tarefas e `roles/run.invoker` para service account no reconciliador. Isso não prova que o callable esteja desprotegido no runtime: o código mantém Auth recente e `enforceAppCheck: true`, portanto retirar o invocador público sem análise quebraria clientes legítimos. A fila permanece `RUNNING` com 1 despacho/s, concorrência 2, 5 tentativas e retry de 24 h; a consulta IAM da fila retornou política vazia. A primeira tentativa incorreta usou GET e recebeu 404; a documentação oficial exige POST para `getIamPolicy`, que retornou 200, evitando registrar falsamente uma falha de acesso. O Scheduler continua habilitado a cada 60 minutos com OIDC da conta padrão. Ambos os repositórios `gcf-artifacts` europeus continuam declarando ação `DELETE` para imagens com mais de 604800 s (7 dias). O perfil Cloudflare `trofia` fechou a lacuna da baseline: o Worker tem `GEMINI_API_KEY` e `RATE_LIMIT_PSEUDONYM_KEY`, ambos `secret_text`, conferidos só por nome/tipo. `npm audit --omit=dev` repetiu 0 vulnerabilidades na raiz, 0 no Worker e 7 moderadas nas Functions (`firebase-admin`, `@google-cloud/storage`, `gaxios`, `qs`, `retry-request`, `teeny-request`, `uuid`); incluindo dev/build, repetiu 3 altas na raiz, 6 altas e 2 moderadas no Worker, 2 altas e 16 moderadas nas Functions. O código confirma Firestore transacional e Cloud Tasks no callable, Firestore recursive delete e Auth delete no processador, e leitura/reenvio de jobs no reconciliador. Após a avaliação do inventário, um candidato local isolado alterou apenas `functions/package.json` e `functions/package-lock.json` para `firebase-admin@14.5.0`; a auditoria inicial do lockfile foi confundida com a da raiz; após `npm ci` no diretório correto das Functions, o resultado comprovado caiu de sete para dois achados moderados (`gaxios`/`uuid`), ainda não zero, mas a instalação e os testes das Functions/emuladores concluíram com sucesso; a suíte completa passou após a liberação das portas, e o CI autenticado real do PR #265 passou no commit `1e3b442`: preflight `36249321257` e gate pesado `36249321278`; não houve deploy ou redução de IAM. As consultas OAuth usaram a sessão existente somente em memória; nenhuma credencial ou política integral foi registrada.
- **PRs/commits relacionados:** inventário preparatório no PR #258, commits `432b8c6`, `a9ddac5` e `730f2b7`, merge `b9ae9ff`; validação administrativa read-only de 26/09/2026. A listagem Cloud Run das duas regiões usadas mostrou somente os três serviços das Functions, todos na identidade padrão; isso não exclui workloads de outros produtos/regiões. A tentativa de listar VMs por Compute Engine `aggregatedList` recebeu HTTP 403, portanto não foi possível afirmar que a conta é exclusiva das Functions; revogar Editor permanece bloqueado por inventário incompleto dos consumidores e simulação de impacto. O audit apontou `firebase-admin@14.5.0` como correção não-major; a versão foi commitada em `1e3b442` no draft PR #265, testada localmente e no CI autenticado, ainda sem merge ou deploy. Fontes oficiais para interpretação: [Cloud Tasks `getIamPolicy` exige POST](https://docs.cloud.google.com/tasks/docs/reference/rest/v2/projects.locations.queues/getIamPolicy), [invocação de Functions Gen2 usa Cloud Run IAM](https://docs.cloud.google.com/functions/docs/securing/managing-access-iam), [permissões Firebase Auth](https://docs.cloud.google.com/iam/docs/roles-permissions/firebaseauth) e [remoção segura de Editor da conta padrão](https://docs.cloud.google.com/compute/docs/access/service-accounts). — **Chat:** Trofia-Principal.

Validação técnica da atualização isolada, com commit e CI autenticado confirmados: `npm audit --omit=dev` nas Functions caiu de sete para dois achados moderados com `firebase-admin@14.5.0` e correção transitiva compatível do lockfile. Os testes focados executaram 33 casos com zero falhas e um skip dependente do emulador; `functions/npm test` executou 62 casos com zero falhas e 12 skips de emulador. O gate `functions/npm run test:emulators` executou depois todos os 74 casos, sem skips nem falhas, incluindo a entrega duplicada de tarefa, o lock concorrente e as negativas previstas das rules. O emulador avisou que o host local usa Node.js 24 enquanto a Function declara Node.js 22; o CI ainda precisa comprovar o runtime de deploy. O `npm test` geral passou no preflight e em 1.465 unitários. A primeira tentativa parou pela ausência de `worker/node_modules/jose` nesta worktree recém-criada; `npm ci --ignore-scripts` no Worker resolveu apenas o ambiente. Na repetição, os unitários passaram e o smoke não começou porque outro processo Playwright legítimo do UI/UX, na worktree `.codex-ui-cam-red-7`, ocupava a porta 8765. Nenhum processo dessa frente foi interrompido. Após a liberação das portas, a terceira execução de `npm test` terminou com código zero: preflight sem avisos, 1.465/1.465 unitários, smokes legado/Vite e cutover 60/60. Os testes autenticados locais sem credenciais ficaram apenas nos skips estruturais previstos, por isso o CI autenticado real passou no SHA publicado `1e3b442` (run `36249321278`); os oito skips legados foram os estruturais esperados. Essas ocorrências anteriores foram gates ambientais, não defeitos atribuídos à atualização.

O primeiro relato de audit zero foi corrigido antes de commit: aquele comando havia sido executado no diretório da raiz, cujo audit de produção já era zero, não em `functions/`. Após `npm ci` das Functions, a medição correta mostrou três achados moderados; uma atualização transitiva compatível do lockfile corrigiu `qs` e deixou dois alertas da mesma cadeia `gaxios@6.7.1 → uuid@9.0.1`. O [aviso oficial](https://github.com/advisories/GHSA-w5hq-g745-h8pq) afeta `uuid.v3/v5/v6` com buffers externos fora do limite; a versão instalada de `gaxios` usa somente `uuid.v4()` sem buffer na montagem de multipart. A avaliação é de exposição limitada neste caminho, não de ausência absoluta de risco. `@google-cloud/storage@8.2.0` continua exigindo `gaxios ^6` e não há versão corrigida na linha 6; um override major de `uuid` não foi imposto sem contrato upstream. A exceção temporária está detalhada no inventário específico. Após a mudança final do lockfile, `functions/npm test` passou novamente (62 executados, 12 skips reservados aos emuladores); a primeira tentativa de emuladores não iniciou por um Java órfão da própria worktree na porta 8080, identificado pelo caminho das rules e encerrado sem afetar outra frente. A repetição passou 74/74 sem skips, e um segundo órfão do mesmo emulador foi removido após confirmar que o processo pai terminara. `npm test` completo passou novamente com preflight, 1.465/1.465 unitários, smokes legado/Vite e cutover 60/60 sobre o lockfile final. O CI autenticado do SHA publicado passou: preflight, 1.465/1.465 unitários, 44/44 testes do Worker, 74/74 das Functions emuladas, 111 smokes legados com oito skips estruturais esperados e 119/119 smokes Vite. O runtime efetivamente publicado em produção continua sem validação porque não houve deploy. O impacto de corrigir a medição e repetir os gates é positivo para a precisão da auditoria; o escopo de produção permanece intocado.
...
- **Data de início:** 26/09/2026.
- **Data de conclusão:** 26/09/2026.
- **Tempo decorrido:** 13 h 2 min 34 s, do primeiro commit `432b8c6` ao merge `b9ae9ff` do PR #258.
- **Minutos de CI:** 1 min 8 s no total (1 min 8 s leves + 0 min pesados, não aplicável a PR somente documental), nos três preflights concluídos do PR #258; o preflight local também terminou sem avisos.
...
- **PRs/commits relacionados:** PR #258; commits `432b8c6`, `a9ddac5` e `730f2b7`; merge `b9ae9ff`; preflight final `36237601755`; nenhuma alteração de produção.
...
- **Data de início:** 17/09/2026.
- **Data de conclusão:** não concluído.
- **Tempo decorrido:** 2 h 33 min 52 s da implementação, do primeiro commit `c6c6707` ao merge `1e9ef55`; o incidente permanece aberto até a prova física pós-hotfix.
- **Minutos de CI:** 1 h 16 min 55 s no total (57 s leves + 1 h 15 min 58 s pesados), incluindo os runs verdes `35267419513`, `35267419620`, `35271507153` e a tentativa cancelada seguida do rerun verde de `35271507208`.
...
- **Validação local e remota:** o teste focado passou 80/80 sem skips. A execução integral autenticada, após carregar somente em memória o App ID/site key públicos do AAB e o debug token local ignorado pelo Git, passou 1.404/1.404 unitários, 103/103 smokes legado com os 8 skips estruturais esperados, 111/111 smokes Vite e 60/60 casos cutover. O login Vite concluiu em 3,8 s, sem `profile-incomplete-existing-account` nem falha de App Check. Tentativas anteriores sem dependências/configuração completas foram descartadas como ambiente inválido e não contabilizadas como validação do produto. No GitHub, o primeiro SHA passou o preflight `35267419513` e o CI autenticado `35267419620`; após o commit documental, o preflight `35271507153` passou e o CI pesado `35271507208`, inicialmente cancelado, foi repetido e terminou integralmente verde no SHA final. O UIUX incorporou `1e9ef55` e confirmou login automatizado sem recorrência antes de encontrar o bloqueio independente de concorrência entre suítes. Essa evidência valida código e bootstrap automatizado, mas não substitui a prova exigida em AAB novo instalado pela Play; por isso o incidente permanece em andamento e sem campo de alinhamento final.
- **Recorrência na prova Play versionCode 21 (20/09/2026):** o mesmo AAB da CAM-RED-4 foi instalado pela faixa interna e conferido antes do primeiro toque: pacote `com.hermegas.trofia`, versionName `0.11.0-beta`, versionCode 21 e instalador `com.android.vending`. O arquivo local correspondente tem SHA-256 `6FEB0F26BD604F05D339C4784F56B505675C2630679742BFB77153E5BFBCEF85`; seu bundle web dentro do AAB é idêntico ao asset preparado no worktree e contém os marcadores do hotfix `firestore-profile-auth-unavailable`, da leitura confirmada de servidor e da classificação `profile-incomplete-existing-account`. O merge `1e9ef55` é ancestral do commit do candidato `ddebe472`; portanto, a recorrência não veio de AAB antigo nem de pacote sem a correção anterior.
- **Execução física e regra de parada:** no Galaxy SM-S938B/Android 16, uma única submissão real com a conta descartável concluiu a etapa Auth e, em aproximadamente cinco segundos, exibiu “Não foi possível carregar seu perfil” com detalhe `profile-incomplete-existing-account`. Não foi usado “Tentar novamente”, não houve segundo login, aumento de timeout, clique forçado, abertura da câmera ou alteração de perfil. O usuário foi desconectado pela ação explícita “Sair”. O app foi finalizado e o aparelho voltou aos valores capturados no preflight: DND `0`, sincronização ativa, timeout de 30 s, `stay_on_while_plugged_in=0`, rotação automática `0` e orientação `0`; ADB e processos auxiliares foram encerrados antes de liberar a desconexão.
- **Verificação do dado após a recorrência:** uma leitura administrativa imediatamente posterior, somente leitura e usando a conta descartável pelo e-mail local ignorado pelo Git, confirmou usuário existente, documento raiz existente com 26 campos e todos os campos exigidos presentes. Os mesmos validadores do runtime retornaram data de nascimento válida, gênero válido, atividade válida, meta válida e `hasRequiredProfileData=true`. Nenhum valor, e-mail, UID, token, header ou dado nutricional foi impresso ou preservado. A consulta de runs também comprovou ausência de qualquer CI autenticado durante a prova física; o último CI pesado da branch havia terminado no dia anterior.
- **Fronteira técnica agora comprovada:** `resolveAuthenticatedProfileGate()` aguarda o token de App Check antes da leitura; uma falha de App Check lançaria um código recuperável e não chegaria a `incomplete-existing`. `fbGetProfileFromServer3()` usa `getDocFromServer()` e transforma erro de rede/permissão em exceção; a tela observada só pode surgir quando essas etapas retornam sem exceção e `hasRequiredProfileData()` recebe um objeto que não satisfaz o contrato. Como o documento real satisfazia o contrato e o AAB contém o hotfix anterior, resta uma inconsistência dentro do cliente entre o snapshot Firestore e o perfil normalizado entregue ao validador. O release não possui telemetria sanitizada por campo nessa fronteira, então ainda não é possível afirmar qual campo foi perdido/normalizado incorretamente nem por qual evento. A correção do PR #229 permanece válida para UID ausente, mas mostrou-se insuficiente para esta segunda origem.
...
- **Tempo decorrido:** 1 h 21 min 1 s.
- **Minutos de CI:** 34 min 55 s no total (25 s leves; 34 min 30 s pesados).
...
- **O que foi feito:** o PR documental #238, commit `a0dfa6c`, merge `7315c8d`, consolidou a recorrência física versionCode 21 antes do código; levou exatamente 18 min 44 s do primeiro commit ao merge e seu único CI leve, run `35519515970`, consumiu 26 s, sem CI pesado. Na implementação D1, `fbGetProfileFromServer3()` passou a transportar, em propriedades não enumeráveis, somente a existência do documento confirmada por `snapshot.exists()` e os tipos observados antes/depois da normalização; `inspectRequiredProfileData()` converte esses estados em um código curto composto apenas por códigos de tipo e cinco resultados booleanos de validação. O gate anexa esse diagnóstico exclusivamente ao resultado `incomplete-existing`, e a tela recuperável/logcat o expõe sem valor de campo, e-mail, UID, token, header ou dado nutricional. Testes com sentinelas secretas provam que o diagnóstico serializado não contém os valores e que as chaves públicas enumeráveis do perfil não mudaram. Os testes focados passaram 96/96 e o preflight passou sem avisos. A suíte local completa final passou com 1.421/1.421 unitários, 48 smokes legado e 48 Vite; em cada matriz, 63 casos autenticados foram pulados porque as credenciais foram deliberadamente removidas deste worktree, e o CI seguro permaneceu responsável por executá-los. O cutover passou 60/60. No GitHub, o preflight `35522846803` passou em 25 s e o CI autenticado `35522846863` passou integralmente, incluindo Worker, Functions e Playwright real. O PR #239 foi mesclado em `ef30eec`; a branch e os arquivos funcionais da CAM-RED-4 permaneceram intocados.
- **Alinhamento:** 100%. A D1 entregou exatamente a observabilidade sanitizada aprovada, preservou o comportamento fail-closed e não adicionou retry, leitura duplicada, cache fallback, timeout maior ou flexibilização de perfil. O impacto é positivo: a próxima ocorrência física poderá localizar a transformação divergente sem expor dados da conta.
- **PRs/commits relacionados:** PR documental #238, commit `a0dfa6c`, merge `7315c8d`, run `35519515970`; PR #239, commit `fd31bc6`, merge `ef30eec`, runs `35522846803` e `35522846863`.
...
- **O que foi feito:** a D1 foi encerrada pelos merges `ef30eec` e `3aba36d`, deixando disponível no bundle o código sanitizado que diferencia existência do documento, tipos brutos, tipos normalizados, tipos entregues ao validador e cinco resultados booleanos, sem valores pessoais. A D2 começou em worktree isolada da `origin/main` `3aba36d`; como o versionCode 21 já havia reproduzido o incidente, o candidato usa versionCode 22 e mantém versionName `0.11.0-beta`. Arquivos de ambiente e assinatura foram usados somente como dados locais ignorados, sem exposição de senhas ou tokens. Três tentativas preliminares falharam corretamente antes da assinatura por configuração local incompleta: ausência do SDK no worktree, Java 22 em vez do toolchain 21 e variáveis públicas Firebase/App Check ausentes no ambiente do Gradle. O build final usou JDK 21.0.10, reconstruiu o Vite, sincronizou sete plugins Capacitor e concluiu 380 tarefas Gradle. O verificador confirmou manifesto/Firebase, `0.11.0-beta` (22), assinatura criptográfica e SHA-256 `E3E48768B929D43F7857E3025C17208A37FE67B7CD388D28D46639A85D2A9B4F`. A cópia de 23.149.570 bytes foi preservada em `.artifacts/releases/trofia-0.11.0-beta-v22-profile-diagnostic.aab`, com hash idêntico ao build. `documentation/operacao/ENTREGA-AAB-PLAY.md` tornou obrigatório informar caminho, versão, hash, assinatura, base Git e notas trilíngues em toda entrega futura de AAB.

  O AAB foi instalado pela faixa interna e conferido antes da prova: pacote `com.hermegas.trofia`, versionCode 22, versionName `0.11.0-beta` e instalador `com.android.vending`. No Galaxy SM-S938B, o preflight registrou timeout de tela em 30 s, permanência ligada ao carregador `0`, DND `0`, sincronização `1`, rotação automática `0` e orientação `0`. A automação ativou temporariamente tela prolongada e DND. A primeira tentativa de ocultar o teclado com Voltar apenas enviou a tarefa do app ao plano de fundo antes de qualquer submissão; o logcat confirmou processo vivo e movimento de tarefa, não crash nem autenticação. O app foi trazido de volta, a senha foi preenchida sem expor credenciais e a única submissão real foi feita pela ação do teclado. O diálogo externo do NordPass foi recusado sem salvar credenciais.
...
- **Status:** não iniciado; condicionado a nova recorrência com diagnóstico causal.
- **Data de início:** não iniciado.
- **Data de conclusão:** não iniciado.
- **Tempo decorrido:** pendente de merge.
- **Minutos de CI:** 0 min; não iniciado.
...
- **O que se planeja fazer:** se uma nova ocorrência produzir o código sanitizado D1, implementar somente a correção sustentada por essa evidência, criar reprodução determinística, executar testes focados, `npm test` completo e CI autenticado real, repetir a prova no AAB Play e atualizar resumo, histórico e inventário. A passagem limpa da D2 permite repetir a CAM-RED-4 com o mesmo critério de parada; não autoriza declarar a intermitência erradicada.
...
- **Minutos de CI:** 2 h 50 min 13 s no total (2 min 44 s leves nos runs `35343456559`/`35352760266`/`35360391261`/`35363788668`/`35441456096` + 2 h 47 min 29 s pesados nos runs `35343456506`/`35352760430`/`35360391365`/`35363788662`/`35441455961`/`35443362435`).
...
- **Infraestrutura remota preparada:** `.github/workflows/authenticated-local-lease.yml` expõe somente `workflow_dispatch` com um UUID opaco, `contents: read`, timeout de 60 minutos e nenhum segredo. Seu grupo de concorrência é exatamente `nutrition-authenticated-suite` com `cancel-in-progress: false`; portanto, quando a F2 passar a dispará-lo e ele estiver ativo, qualquer CI novo ficará enfileirado. Nesta F1 o workflow é deliberadamente apenas infraestrutura: o guia atual registra que ainda não deve ser tratado como lease adquirido automaticamente, evitando documentar como pronto um comportamento que depende de ele existir primeiro na `main`.
- **Validação concluída até o primeiro checkpoint:** cinco repetições mobile consecutivas sobre a `origin/main` limpa passaram, somadas a uma execução inicial e a uma execução posterior já usando o coordenador (6/6 antes da mudança e 1/1 depois). Os nove testes focados do coordenador/App Check passaram; o arquivo de lock foi comprovadamente removido após o Playwright. A suíte integral terminou verde com 1.409/1.409 unitários, 103/103 smokes legado com 8 skips estruturais esperados, 111/111 Vite e 60/60 cutover. O CI autenticado `35343456506` passou em 30 min 53 s e o preflight `35343456559` em 45 s. Esta evidência valida o diagnóstico e a proteção inicial; o novo workflow remoto e seu HEAD ainda precisam de validação antes do merge.
- **Validação local da infraestrutura remota:** os 14 testes focados de coordenador, App Check e workflows passaram. O teste de workflow exige acionamento manual, UUID obrigatório, permissões mínimas, grupo compartilhado, `cancel-in-progress: false`, timeout e ausência de referências a segredos. Depois da inclusão do workflow e do guia, a suíte integral do novo HEAD terminou verde com preflight sem alertas, 1.410/1.410 unitários, 103/103 legado com os 8 skips estruturais esperados, 111/111 Vite e 60/60 cutover. Resta o CI remoto do novo HEAD antes do merge da F1.
- **Achado do primeiro CI do novo HEAD:** o run pesado `35352760430` confirmou preflight, 1.410 unitários, Worker, Functions e todos os quatro fluxos críticos de persistência em desktop/mobile, mas falhou num teste visual de `SearchableChoiceField`. O artefato mostrou o app ainda na aba Alimentos, com o editor de uma refeição salva aberto e o ingrediente de fixture alterado sem salvar; em seguida o roteiro aguardou por 180 s o botão “Registrar suplemento”, que só existe no Diário. Isso não indicava falha do produto nem da coordenação: o teste havia encerrado somente a folha do seletor e deixado o editor pai sujo antes de tentar a troca de aba.
- **Correção da fragilidade visual:** depois de validar a seleção pesquisável, o roteiro agora aciona explicitamente “Cancelar” no editor da refeição salva e comprova que seu gatilho desapareceu antes de navegar ao Diário. Não foi adicionado retry, clique forçado novo nem timeout. O recorte autenticado passou em setup, desktop e mobile (3/3 em 30,4 s). A alteração fica registrada nesta fatia porque foi descoberta pelo gate final, mas é independente da causa raiz de concorrência entre suítes.
- **Revalidação integral após a correção visual:** uma primeira tentativa local foi descartada antes da matriz Vite porque o comando manual havia fornecido App ID e token debug, mas omitido a site key pública do reCAPTCHA Enterprise; a própria inicialização fail-closed exibiu `app-check-initialization-failed`, sem evidência de regressão. Com a configuração pública completa carregada somente em memória, `npm test` passou preflight, 1.410/1.410 unitários sem skips, 103/103 legado com os mesmos 8 skips estruturais, 111/111 Vite e 60/60 cutover. A F1 volta assim ao único gate pendente: repetir o CI remoto do HEAD corrigido.
- **Gate remoto final da implementação:** o preflight `35360391261` passou em 22 s e o CI autenticado `35360391365` passou integralmente em 31 min 46 s, cobrindo preflight, 1.410 unitários, Worker, Functions e toda a matriz Playwright. A F1 fica pronta para revisão e merge; a F2 permanece não iniciada porque sua aquisição automática depende de o workflow desta fatia já existir na `main`.
- **Achado do gate redundante do commit documental:** o preflight `35363788668` passou, mas o CI `35363788662` terminou com 102 smokes aprovados, os mesmos 8 skips estruturais e uma falha desktop no contrato visual do `SearchableChoiceField`. A asserção comparava a posição da busca antes e depois de rolar somente a lista; a primeira medida ficou em `-67,75 px` e a segunda em `335,25 px`. O artefato mostrou que a busca estava realmente fora do viewport, portanto a falha não foi descartada como mero arredondamento ou corrigida aumentando tolerância.
- **Causa técnica do seletor fora da tela:** o `SearchableChoiceField` renderizava seu overlay `position: fixed` dentro do cartão `pantry-meal-templates`. Esse cartão usa `backdrop-filter`, propriedade que cria um bloco de referência para descendentes fixos no Chromium. Assim, no desktop o overlay era posicionado em relação ao cartão rolável, e não à janela; o foco assíncrono conseguia ainda deslocar a medição no meio do teste. No mobile, a combinação de dimensões não expunha o mesmo corte.
...
- **Gate remoto definitivo e merge:** o preflight `35441456096` passou em 24 s. O CI autenticado `35441455961` passou integralmente em 38 min 59 s, incluindo preflight, 1.412 unitários, Worker, Functions e toda a matriz Playwright; o upload de diagnósticos foi corretamente pulado por não haver falha. O PR #231 foi retirado do draft e mesclado em `763beec`, tornando o workflow de lease disponível na `main` para a F2. O CI pós-merge da `main` `35443362435` repetiu o gate integral e passou em 34 min 57 s; ele foi incluído no total definitivo da fatia.
...
- **Data de conclusão:** 19/09/2026.
- **Tempo decorrido:** 1 h 8 min 12 s, do primeiro commit `0b0c110` ao merge `d56480e`.
- **Minutos de CI:** 1 h 37 min 26 s no total (1 min 19 s leves nos runs `35447128772`/`35448749624`/`35450534290` + 1 h 36 min 7 s pesados nos runs `35447128752`/`35448749636`/`35450534288`).
...
- **O que foi feito:** a worktree isolada `codex/authenticated-lease-f2` foi criada sobre `origin/main` no merge `d206df3`, depois que o PR #231 tornou o workflow de lease disponível e o PR #232 concluiu seu registro pós-merge.
- **Aquisição automática implementada:** fora do GitHub Actions, o coordenador mantém o lock atômico local e a recusa rápida diante de CI já ativo; em seguida usa `gh workflow run` para disparar `authenticated-local-lease.yml` em `main` com UUID opaco, localiza exclusivamente o run cujo `displayTitle` contém esse UUID e só permite o login depois que o status remoto passa a `in_progress`, prova de que o workflow adquiriu `nutrition-authenticated-suite`. O UUID não contém credencial, UID, token nem dado do usuário.
- **Liberação fail-closed implementada:** o teardown cancela o run público por ID, consulta seu estado até `completed` e somente então libera o lock local. Erros de dispatch, listagem, JSON inválido, leitura, cancelamento ou timeout geram códigos sanitizados e interrompem a suíte. A função de liberação é idempotente; o workflow conserva timeout remoto de 60 minutos para recuperar processo local morto. Dentro do Actions, nenhum lease adicional é criado porque o próprio workflow `CI` já possui o grupo compartilhado.
...
- **Prova controlada lease→CI:** o lease `35448711416` adquiriu `nutrition-authenticated-suite` às `14:25:53Z`. O push documental disparou o CI `35448749636` às `14:26:36Z`; ele permaneceu `pending` durante toda a posse do lease, enquanto o preflight independente `35448749624` pôde passar normalmente em 33 s. O teardown cancelou o lease e confirmou `completed/cancelled` às `14:31:14Z`; sem novo dispatch nem retry, exatamente o mesmo CI iniciou às `14:31:16Z`, 2 s depois, e passou integralmente em 29 min 56 s de execução. Isso comprova a exclusão mútua também no sentido local→CI e preserva paralelismo para verificações que não usam a conta.
- **Resultado final do PR e da `main`:** os dois HEADs do PR passaram integralmente. Não houve skip além dos 8 casos estruturais já documentados do legado, nenhuma decisão adicional, nenhuma ação manual e nenhum comportamento inesperado. O PR #233 foi mesclado em `d56480e`; em seguida, o preflight pós-merge `35450534290` passou em 25 s e o CI autenticado da `main` `35450534288` repetiu com sucesso todo o gate em 35 min 57 s.
...
- **Data de início:** 19/09/2026.
- **Data de conclusão:** 19/09/2026.
- **Tempo decorrido:** 1 h 37 min 28 s, do primeiro commit `33590a3` ao merge `53e8fd9`.
- **Minutos de CI:** 1 h 45 min 48 s no total (2 min 49 s leves: quatro preflights e build/deploy/smoke Pages; 1 h 42 min 59 s pesados: primeira tentativa falha, repetição verde e CI pós-merge).
...
- **O que foi feito:** o workflow pós-merge `35452419932` concluiu build e deploy com sucesso, mas os quatro casos desktop/mobile de `app-orchestration.spec.js` receberam `app-check-token-unavailable`. O teste usa login, storage e perfil simulados para validar a orquestração da UI; como `playwright.pages.config.js` não executa o setup autenticado e o fixture só instalava App Check quando havia e-mail/senha, o novo gate real abortava antes dos stubs de perfil. A repetição isolada do job produziu o mesmo resultado, descartando atraso de propagação. O fixture passou a instalar o debug provider registrado também no modo Pages sem credenciais, mas só cria a rota Firestore e exige token trocado no modo autenticado; assim o smoke de UI recebe App Check sem ganhar acesso indevido ao banco. O workflow injeta o segredo apenas no job efêmero de verificação, nunca no build/deploy, e o Playwright desativa traces quando esse segredo existe para impedir persistência acidental. Testes unitários cobrem separação dos dois modos e fail-closed autenticado; o teste direto contra o Pages passou 4/4 e a suíte local completa passou 1.418/1.418 unitários, 103 legado com 8 skips estruturais esperados, 111/111 Vite e 60/60 cutover. O primeiro CI do PR #235 (`35455830093`) confirmou preflight, unitários, Worker, Functions, 103 legado com 8 skips esperados e 110/111 Vite. A única falha foi o caso GA desktop: o screenshot mostra a entrada correta já renderizada no Diário, mas `readDailyLog()` não a encontrou no Firestore dentro de 30 s. Como o mesmo caso passou localmente, os outros sete fluxos críticos desktop/mobile passaram e a alteração não toca persistência, a ocorrência foi registrada e revalidada sem aumentar timeout, inserir retry interno ou relaxar expectativa. O CI integral `35458919766` passou na repetição, seguido pelo CI pós-merge `35460807918`. O Pages pós-merge `35462394508` concluiu build, deploy, verificação de HTML/MIME e os quatro casos publicados de `app-orchestration.spec.js` em desktop/mobile, confirmando o conserto no ambiente que originalmente falhava.
- **Alinhamento:** 100%. O escopo aprovado foi entregue sem alterar runtime, enforcement ou acesso ao Firestore. A primeira falha GA exigiu apenas revalidação do gate já estrito e teve impacto neutro no produto; a investigação preservou evidência e evitou mascarar uma falha alheia ao harness Pages.
- **PRs/commits relacionados:** descoberta no run Pages `35452419932`, após o merge `d56480e`; PR #235, commits `33590a3`, `d5182e6` e `6507e01`, merge `53e8fd9`; runs `35455830127`, `35455830093`, `35458919764`, `35458919766`, `35458972574`, `35460807949`, `35460807918` e `35462394508`. — **Chat:** Trofia-Principal.
...
- **O que foi feito:** os seis casos falharam entre 16:09:12Z e 16:15:17Z, apenas 11–17 minutos após a aquisição do lease; portanto, o timeout remoto de 60 minutos não causou o loading. Os artefatos mostraram dois resultados da mesma corrida: alguns contextos permaneceram no overlay “Entrando...”, enquanto outro já havia sido devolvido à tela pública de login em espanhol. Em `src/App.jsx`, a restauração modular aguardava `authStateReady()`, mas um timer legado de 8 s executava `fbSignOut()` e alterava a UI em paralelo. Se a restauração ultrapassasse esse limite sob carga, bootstrap, leitura de perfil e logout passavam a disputar a mesma sessão válida. Isso explica os sucessos intercalados e por que a falha podia tanto manter loading quanto remover a navegação.

  O run `36024006073` foi adquirido corretamente às 15:58:05Z. O teardown recebeu `authenticated-smoke-remote-lease-cancel-failed`, abandonou a liberação após a primeira falha do comando e o workflow continuou dormindo até o timeout de segurança, às 16:58:06Z. A reprodução limpa não encontrou CI concorrente: os seis arquivos afetados passaram 9/9 uma vez e 25/25 em três repetições adicionais com credenciais descartáveis e App Check completo; os leases novos foram cancelados normalmente. A intermitência não voltou nessas quatro execuções, mas a corrida destrutiva e a fragilidade de cancelamento são objetivas e cobertas sem depender da lentidão real.
