...
- **Correção definitiva proposta, ainda não publicada:** manter validação integral em creates; em updates, permitir alterar somente a allowlist canônica e validar estritamente o tipo de cada campo efetivamente modificado. Campos históricos podem sobreviver apenas inalterados. A entrada mantém allowlist, identidade, tipos centrais e validação dos nutrientes que alimentam os totais; o snapshot aninhado mantém allowlist e identidade textual sem duplicar as 18 verificações nutricionais. O emulador cobre uma entrada completa, cinco exclusões, marcador e `_dailyDates` no mesmo batch, com 8/8 cenários verdes.
...
- **Status:** em validação.
- **Data de conclusão:** não concluída; diagnóstico e correção iniciados em 02/09/2026.
