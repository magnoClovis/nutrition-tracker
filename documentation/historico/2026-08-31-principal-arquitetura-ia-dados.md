...
- **O que foi feito:** em 12/09/2026, durante a fase 4 do rollout C14-C no Pages, uma conta antiga com perfil completo recebeu o modal “Completar perfil nutricional”; após F5 o modal desapareceu e Descrever prato, Reconhecer por foto e Avaliar refeição com explicação funcionaram. O diagnóstico comprovou uma categoria distinta da C14-A: não houve exceção mascarada, mas uma leitura `getDoc()` aceita como sucesso a partir de cache antigo. Além disso, inicializar o provedor não garantia token pronto, o adaptador aceitava o dummy token não vazio que o SDK pode devolver junto de um erro, o timeout de autenticação abrangia indevidamente a espera da atestação e login/cadastro compartilhavam o mesmo caminho para abrir o modal. O hotfix em validação rejeita resultados/dummy tokens, espera um token real antes da primeira leitura protegida, usa `getDocFromServer()` para o gate, remove App Check do timeout que poderia derrubar uma sessão válida, mostra erro recuperável em falha e só retorna `requires-completion` quando `isNewAccount === true`. A cobertura inclui token atrasado/inválido, cache incompleto, servidor indisponível, repetição equivalente a F5 e distinção entre conta antiga e nova. Nenhum AAB foi solicitado e o Worker permanece em `observe` até o encerramento desta correção.
- **PRs/commits relacionados:** branch `codex/c14-c-profile-readiness`; PR/commit pendentes até a validação completa. — **Chat:** Trofia-Principal.
