...
- **O que foi feito:** a D1 foi encerrada pelos merges `ef30eec` e `3aba36d`, deixando disponível no bundle o código sanitizado que diferencia existência do documento, tipos brutos, tipos normalizados, tipos entregues ao validador e cinco resultados booleanos, sem valores pessoais. A D2 começou em um único worktree criado diretamente da `origin/main` `3aba36d`; como o versionCode 21 já foi distribuído e reproduziu o incidente, o candidato usa versionCode 22 e mantém versionName `0.11.0-beta`. `.env.local`, `google-services.json`, `keystore.properties` e `local.properties` foram copiados somente como arquivos ignorados, com hashes conferidos quando aplicável; a chave de assinatura absoluta referenciada foi confirmada existente sem ler ou exibir senhas. Três tentativas preliminares falharam corretamente antes da assinatura por configuração local incompleta: ausência do SDK no worktree, Java 22 em vez do toolchain 21 e variáveis públicas Firebase/App Check ausentes no ambiente do Gradle. O build final usou o JDK 21.0.10 do Android Studio e, somente em memória, os mesmos parâmetros públicos já embarcados no AAB 21 validado; o Vite foi reconstruído e verificado, os sete plugins Capacitor foram sincronizados e 380 tarefas Gradle concluíram. O verificador confirmou manifesto/Firebase, `0.11.0-beta` (22), assinatura criptográfica e SHA-256 `E3E48768B929D43F7857E3025C17208A37FE67B7CD388D28D46639A85D2A9B4F`. Uma cópia de 23.149.570 bytes foi preservada em `.artifacts/releases/trofia-0.11.0-beta-v22-profile-diagnostic.aab`; o hash da cópia coincide com o artefato de build. VersionCode e arquivos gerados do Android permanecem alterações locais fora do commit, como previsto. Foi criado `documentation/operacao/ENTREGA-AAB-PLAY.md`, tornando obrigatório em todo AAB futuro informar pasta e arquivo exatos, versão, hash, assinatura, base Git e notas trilíngues prontas no formato do Play Console. O upload Play e a única prova física sanitizada ainda não ocorreram.
