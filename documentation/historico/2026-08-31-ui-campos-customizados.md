...
- `CheckboxField` mantém um `input type="checkbox"` real associado ao texto por `label`. O input é apenas retirado visualmente do fluxo; continua focável, expõe `checked`, `disabled`, `required`, `name`, `value` e `aria-describedby`, e traduz o evento nativo para `onChange(checked, event)`.
